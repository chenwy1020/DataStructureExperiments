...
HashTable *InitHashTable(const unsigned int tablesize)
{
	...
	// 分配和初始化数据元素单链表的头结点。
	hh->head = (Node *)malloc(sizeof(Node) * (hh->tablesize));
	memset(hh->head, 0, sizeof(Node) * (hh->tablesize));
...
int Delete(HashTable *hh, unsigned int key)
{
	...
	while ((pp->next != NULL) && (pp->next->data.key != key))
	{
		pp = pp->next;
	}
...
int Insert(HashTable *hh, Element *ee)
{
	...
	memcpy(&qq->data, ee, sizeof(qq->data));
